...
function Homepage() {
  ...
  const counterBox = [
    ...
    { counter: "100", counterBox_desc: "Dream Weddings Delivered", suffix: "%" },
  ...
  const feedbackResponsive = {
    desktop: { breakpoint: { max: 3000, min: 992 }, items: 3, slidesToSlide: 1 },
    ...
    <>
      ...
      <Box className="homepage_wrapper">
            ...
            {bannerImage.map((data, index) => (
              <Box key={index} sx={{ position: "relative", width: "100%" }}>
                ...
                <Box className="banner_content">
                  <span className="eyebrow">Event Management &amp; Tent House</span>
                  <h3>{data.heading}</h3>
                  <h5>{data.subheading}</h5>
                  <p>{data.desc}</p>
                  <Button className="cta_button" component={RouterLink} to="/contact">
                    Contact Us
                  </Button>
                </Box>
              </Box>
        ...
        <Box className="about_content">
          <Grid container spacing={3} className="row">
            <Grid xs={12}>
              ...
              <p>
                At Balaji Tent House, we turn ordinary venues into magical celebrations. With 9+ years of
                experience, vibrant decor, and unmatched service, we've been crafting dream weddings for
                countless happy couples.
                <RouterLink to="/about" className="read-more">Read More...</RouterLink>
              </p>
            </Grid>
          </Grid>
        </Box>
        ...
        <Box className="counter_section">
          <Grid container spacing={3} className="row">
            <Grid xs={12} sm={6} sx={{ alignSelf: "center" }}>
            ...
            <Grid xs={12} sm={6}>
              <img src="https://images.pexels.com/photos/1042152/pexels-photo-1042152.jpeg" alt="Wedding decor setup by BalaJi Tent House" className="img-fluid" />
            </Grid>
          </Grid>
        </Box>
              ...
              >
                {client_feedback.map((data, index) => (
                  <Box className="testimonial_box" key={index}>
                    ...
                    <p>{data.clientDesc}</p>
                    <Stack spacing={1}>
                      <Rating name="half-rating-read" defaultValue={data.clientRating} precision={0.5} readOnly />
                    </Stack>
                  </Box>
                ))}
              </Carousel>
        ...
        <Box className="cta_strip">
          <Grid container spacing={3} className="row" alignItems="center">
            ...
            <Grid xs={12} sm={4} className="cta_strip_action">
              <Button className="cta_button" component={RouterLink} to="/contact">
                Get a Free Quote
              </Button>
            </Grid>
